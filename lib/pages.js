...
/**
 * Waits until there are no network connections for at least 500 ms, but at most
 * until the timeout.
 * @param {Page} page - A Playwright
 * {@link https://playwright.dev/docs/api/class-page|Page} object
 * @param {number} timeout - Maximum wait time in milliseconds, use 0 for no
 * timeout
 * @returns {Promise<void>} - A promise to wait on
 * @see https://playwright.dev/docs/api/class-page#page-wait-for-load-state
 * @memberof pages 
 */
const waitForNetworkIdleMax = async function(page, timeout) {
  return page.waitForLoadState('networkidle', {timeout: timeout})
      .catch(error => { log.info("pages.waitForNetworkIdleMax.exceeded"); })
}
module.exports.waitForNetworkIdleMax = waitForNetworkIdleMax;
