 ...
 * @returns {object} - Contains a property per snapshot with the return value of
 ...
 * @returns {string} - The HTML DOM or `false` if `options` is `false`
 * @memberof pages 
 */
const takeDomSnapshot = async function(page, options = {}) {
  if (options === false) { return null; } // do not execute
 ...
 * @returns {string} - The nodes in JSONL format (one node per line) with these
 * properties:
...
const takeNodesSnapshot = async function(page, options = {}) {
  ...
  if (opts.path !== undefined) {
    fs.writeFileSync(opts.path, nodesSnapshot);
  }
 ...
 * function
 * @returns {Promise<Buffer>} - The screenshot as buffered image or `false` if
 * `options` is `false`
 * @memberof pages 
 */
...
const takeViewportSnapshot = async function(page, options = {}) {
  if (options === false) { null; } // do not execute
