...
const instantiateBrowserContext = async function(contextName,
  ...
  if (showBrowserProcesses !== null) {
    browserContext.on("close", () => {
      ...
      for (const showBrowserProcess of showBrowserProcesses) {
        showBrowserProcess.kill();
      }
    });
  }
  if (warcProcess !== null) {
 ...
 * data to
 * @returns {Promise<array<object>|null>} If a server was started, the server's
 * process objects, otherwise <code>null</code>
 */
const prepare4ShowBrowser = async function(
  ...
  if (runOptions[runopts.SHOW_BROWSER] !== false) {
    ...
    const logDirectory = outputContextDirectory;

    const opts = runopts.SHOW_BROWSER;

    const xvfbProcess = xvfb.start(logDirectory,
      opts[runopts.SHOW_BROWSER_WIDTH], opts[runopts.SHOW_BROWSER_HEIGHT]);
    await (new Promise(resolve => setTimeout(resolve, 1000)));
    const vncProcess = vnc.start(logDirectory, opts[runopts.SHOW_BROWSER_PASSWORD]);
    await (new Promise(resolve => setTimeout(resolve, 1000)));
    return [xvfbProcess, vncProcess];
  } else {
    return null;
  }
}
