...
const run = async function(
  ...
  const opts = Object.assign({}, cli.RUN_OPTIONS_DEFAULT, runOptions);
...
const prepare4Insecure = function(browserContextOptions, runOptions) {
  if (runOptions[cli.RUN_OPTION_INSECURE]) {
...
const prepare4Proxy = function(browserContextOptions, runOptions) {
  const value = runOptions[cli.RUN_OPTION_PROXY];
  if (value !== false) {
    log.info({
      proxy: value
...
const prepare4ShowBrowser = function(browserContextOptions, runOptions) {
  if (runOptions[cli.RUN_OPTION_SHOW_BROWSER]) {
    log.info("scripts.prepare4ShowBrowser");
...
const prepare4Har = function(
    browserContextOptions, runOptions, outputContextDirectory) {
  if (runOptions[cli.RUN_OPTION_HAR]) {
...
const prepare4Tracing = function(
    browserContextOptions, runOptions, outputContextDirectory) {
  if (runOptions[cli.RUN_OPTION_TRACING]) {
...
const prepare4Video = function(
    browserContextOptions, runOptions, outputContextDirectory) {
  const value = runOptions[cli.RUN_OPTION_VIDEO_SCALE_FACTOR];
  if (value !== false) {
    let scaleFactor = cli.RUN_OPTION_VIDEO_SCALE_FACTOR_DEFAULT_IF_SET;
 ...
 * @param {string} outputContextDirectory - The directory to write the context's
 * data to
 * @returns {object|null} If a server was started, the server's process object,
 * otherwise <code>null</code>
 */
const prepare4Warc = async function(
    browserContextOptions, runOptions, outputContextDirectory) {
  if (runOptions[cli.RUN_OPTION_WARC]
      && (runOptions[cli.RUN_OPTION_REPLAY] === cli.RUN_OPTION_REPLAY_NOT)) {
    const record = true;
    const pywbDirectory =
...
const prepare4Replay = async function(
    browserContextOptions, runOptions, outputContextDirectory,
    scriptDirectory, inputDirectory) {
  if (runOptions[cli.RUN_OPTION_REPLAY !== cli.RUN_OPTION_REPLAY_NOT]) {
    ...
    // start
    const record =
      (runOptions[cli.RUN_OPTION_REPLAY] === cli.RUN_OPTION_REPLAY_READ_WRITE);
    return await preparePywb(browserContextOptions, record, pywbDirectory);
  } else {
    return null;
  }
...
const startTracing = async function(
  ...
  if (runOptions[cli.RUN_OPTION_TRACING]) {
    ...
    browserContext.on("close", async function() {
      await compressing.zip.compressDir(
        path.join(outputContextDirectory, BROWSER_CONTEXT_TRACE_DIRECTORY),
        path.join(outputContextDirectory, "trace.zip"),
      );
    });
    return true;
  } else {
...
const startUnrandomize = async function(browserContext, runOptions) {
  const value = runOptions[cli.RUN_OPTION_UNRANDOMIZE];
  if (value === cli.RUN_OPTION_UNRANDOMIZE_NOT) {
    return false;
  } else if (value == cli.RUN_OPTION_UNRANDOMIZE_CONSTANT) {
    ...
    throw new Error("Unsupported value for run option '"
      + cli.RUN_OPTION_UNRANDOMIZE + "': '" + value + "'");
  }
}
