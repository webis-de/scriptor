 ...
 */
const SHOW_BROWSER_PORT_DEFAULT = 5920;
...
const getRunOptions = function(options) {
  ...
  if (options.showBrowser !== undefined) {
    const width = options.showBrowser.width === undefined
      ? SHOW_BROWSER_WIDTH_DEFAULT : options.showBrowser.width;
    const height = options.showBrowser.height === undefined
      ? SHOW_BROWSER_HEIGHT_DEFAULT : options.showBrowser.height;
    const password = options.showBrowser.password === undefined
      ? SHOW_BROWSER_PASSWORD_DEFAULT : options.showBrowser.password;
...
const getDockerExposedPorts = function(options) {
  ...
  if (options.showBrowser !== undefined) {
    ...
    exposedPorts.push(bind + ":" + port + ":5920");
  }
  return exposedPorts;
}
