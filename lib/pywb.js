 ...
 * TODO
 */
const importWarcs = async function(
    pywbDirectory, warcs,
    collectionName = COLLECTION_NAME_DEFAULT) {

  const commandLog = await processHelpers.openWriteStream(
    path.join(pywbDirectory, "pywb-import.log"), "a");
  const commandErr = await processHelpers.openWriteStream(
    path.join(pywbDirectory, "pywb-import.err"), "a");

  warcFiles = [];
  if (warcs.isDirectory()) {
    fs.readdirSync(warcs, { withFileTypes: true })
      .filter(warcFile => !warcFile.isDirectory())
      .forEach(warcFile => {
        warcFiles.push(path.resolve(path.resolve(), warcs, warcFile));
      });
  } else {
    warcFiles.push(path.resolve(path.resolve(), warcs));
  }

  const command = "wb-manager";
  const args = ["add", collectionName];
  for (warcFile of warcFiles) { args.push(warcFile); }
  const pywbOptions = {
    cwd: pywbDirectory,
    timeout: timeoutInMilliseconds,
    stdio: [ 'pipe', commandLog, commandErr ]
  ...
  log.info({ command: command, args: args }, "pywb.importWarcs.spawn");
  const wbManager = child_process.spawnSync(command, args, pywbOptions);
  if (wbManager.status !== 0) {
    throw new Error("Failed to import into wayback collection");
  }
}
